    ...
    for filename in os.listdir(input_dir):
        if filename.endswith(".csv"):
            ...
            if not all(col in df.columns for col in columns_to_extract):
                print(f"Required columns missing in {filename}. Deleting the file.")
                os.remove(filepath)  # Use the full file path to delete the file
                continue

            # Extract the data for peak detection
            ...
            plt.plot(x_accel_data, label='Accel X')
            plt.plot(peaks, x_accel_data[peaks], "x", label='Detected Peaks')
            plt.plot(y_accel_data, label='Accel Y')
            plt.plot(z_accel_data, label='Accel Z')
...
# Directories
input_directory = "exercise_wise_ml_pipelines/skipping/data"
segment_plot_directory = "exercise_wise_ml_pipelines/skipping/segment_plots"
segment_csv_directory = "exercise_wise_ml_pipelines/skipping/segment_csvs"
peak_plot_directory = "exercise_wise_ml_pipelines/skipping/peak_plots"
