...
with open('exercise_wise_ml_pipelines/ball_bounce_and_catch/ball_bounce_and_catch_anomaly_detector.tflite', 'wb') as f:
...
def get_tflite_predictions(interpreter, X):
    ...
    results = []
    for sample in X:
        interpreter.set_tensor(input_details[0]['index'], sample.reshape(1, 63).astype(np.float32))
        interpreter.invoke()
